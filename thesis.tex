...
\begin{table}
 ...
 \begin{tabular}{l*{5}{c}}
 	...
 	\bottomrule
 \end{tabular}
 \caption{Extant models for capsule endoscopy organ labelling using the RI dataset}
 \label{tab:models}
\end{table}
...
Now that we have the tools ready, we can begin the task of constructing search spaces and evolving search strategies. We begin by selecting a model to serve as a point of departure for our search space design. For this purpose, we chose the model proposed by Palomero Bernardo et al. \cite[34]{bernardo25}, which was later modified by Bause et al. with a modified hardware reshaping of the input as well as an additional first convolutional layer with an $8\times 1$ that converts the dimensions of the input into a square shape, since their new hardware reshaping delivers a rectangular input to the CNN \cite[37]{bause25}. The reason that we do not use Localization Net (the model proposed by Bause et al.) model, despite its superior performance (see Table \ref{tab:models}). This is because the current version of HANNAH's NAS framework does not support asymmetric kernels, which would be necessary to construct structures such as Localization Net's first convolutional layer. The early search spaces that we built used the same parameters used by Palomero Bernardo et al. for their model: 

\begin{minted}{python}
	out_channels = IntScalarParameter(min=32, max=256, step_size=16,
	name='out_channels')
	kernel_size = CategoricalParameter(name='kernel_size', choices=[1, 3])
	stride = CategoricalParameter(name='stride', choices=[1, 2])
	groups = CategoricalParameter(name="groups", choices=[1, 4, 8, 16])	
\end{minted} 

We also chose the number of layers to be between 6 and 8, closely matching the original model of Palomero Bernardo et al \cite[37]{bernardo25}. When this did not achieve satisfactory results, we modified, we broadened our parameter choices as follows:

\begin{minted}{python}
	out_channels = IntScalarParameter(min=16, max=256, step_size=16,
	name='out_channels')
	kernel_size = CategoricalParameter(name='kernel_size', choices=[1, 3, 5])
	stride = CategoricalParameter(name='stride', choices=[1, 2])
	groups = CategoricalParameter(name="groups", choices=[1, 4, 8, 16])	
...
We also added a constraint to our search design: \py{conv.cond(out_channels >= in_channels)}, so that the number of channels in every sampled network represent a monotonically rising function. Additionally, we introduced weight constraints within our search space. Ideally, it would have been better to introduce one change at a time and run a sufficiently long NAS to observe the effect of each singular change. However, given the paucity of time and the fact that our first priority is to find good models, we had to make multiple co-terminus modifications to our search design and strategy.

Another important aspect of search strategy has to do with NAS hyperparameters. Since NAS is time-consuming, we limited the training of each model to 10 epochs, typically with 10\% of the training set, although we have reduced this further for some searches. We also limited the validation set for some of our searches. Eventually, we settled on the following strategy: use the full validation set, but validate only once per model, i.e. we set \texttt{check\_val\_every\_n\_epoch: 10} in the configs. Additional details on the configurations used for our experiments can be found in Chapter \ref{ch:results}.

We conducted multiple neural architecture searches with different search space designs and NAS hyperparameter configurations instead of one long NAS. The reason is that the number of models in even a relatively constrained search space can easily exceed the number of atoms in the known universe \cite{atoms}, \cite{reiber26}. The number of bad and inefficient models within such a huge search space will inevitable be very large. Such a NAS would take a long time and consume an enormous amount of computational resources and still have a fairly low probability of delivering a good model. And since we had no \textit{a priori} knowledge of a more limited design space that would deliver good results, we had to settle for a process of trial and error.

Our initial experiments were classical CNN-based searches. Later we integrated HMMs into our NAS to find a CNN that delivers good predictions that can be effectively improved on by NAS. For this, we had to integrate the existing HMM machinery into our NAS framework. The implementation of this HMM-NAS integration is described in the next section.


\section{Integrating HMM into the NAS Framework}
A Command-Line Interface- or CLI-based HMM programme already exists in HANNAH that can be applied to an already trained and tested CNN. But not all CNNs deliver predictions that can be improved upon by an HMM. For instance, if the predictions of a CNN are very noisy, the HMM will often struggle to correct the prediction sequence and may even deliver results with \textit{worse} accuracy. This is because a CNN can, at any time, move from an incorrect prediction of a later organ to predict the correct, earlier organ at a subsequent timestep. The HMM can only do this between window transitions. HMM predictions therefore tend to be much smoother, but can  be led astray by noisy CNN predictions.

We want to test the hypothesis that conducting NAS with an integrated HMM framework and using an HMM performance measure as our optimising objective will likely produce CNN models that make less noisy predictions. These can then be more effectively improved upon by the HMM, thus delivering predictions that are both more accurate \textit{and} less noisy.

In order to accomplish this, we first created a separate python programme file called \py{hmm_nas.py} with a callable function \py{run_hmm}. This function runs the original HMM programme and returns the revised performance metrics produced by it after application of the HMM programme on the input CNN predictions. We call this function from the NAS framework's model trainer, using the experiment configurations to allow the user to pass values for key HMM parameters, such as \py{window_size} and \py{quantization}. We then update the NAS results with the HMM metrics returned by \py{run_hmm}.

\begin{minted}{python}
	'''NAS-HMM Integration'''
	
	if config.get("hmm", None) is not None:
		hmm_metrics = run_hmm(cnn_output_dir=os.getcwd(),
				window_size=config.hmm.window_size,
				class_of_interest=config.hmm.class_of_interest,
				quantization=config.hmm.quantization,
				nas=True if config.get("nas", None) is not None else False
	)
	
		res.update(hmm_metrics)
\end{minted}

We insert the same integration code in HANNAH's general training module, so that the HMM programme can be automatically run after retraining and testing a model derived from a NAS experiment. Note that the boolean \py{nas} is used to access the correct CNN output files depending on whether we are running a NAS or retraining and testing a NAS model.


\begin{minted}{python}
if nas:
	path_train = cnn_output_dir + '_cnn_train_output'
	path_val = cnn_output_dir + '_cnn_val_output'
	
	df_train = pd.read_csv(path_train)
	df = pd.read_csv(path_val)
else:
	path_train = cnn_output_dir + '_cnn_train_output'
	path_test = cnn_output_dir + '_cnn_test_output'
\end{minted}

The results of NAS experiments run both with and without the integrated HMM framework are discussed in the next chapter.


\section{Conclusion}

\chapter{Result Exploration and Analysis}\label{ch:results}
